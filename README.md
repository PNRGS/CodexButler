...
The mobile Settings screen shows `Mode`, `Bridge`, and diagnostic detail from `/health`. If proxy mode reports `Bridge: unavailable` with an `app-server-control` socket error, the installed Codex Desktop process is not exposing a proxy target for Concierge on that machine.
...
3. Start with `CODEX_MOCK_MODE=true`, then run `pnpm dev:backend`.
...
8. Tap "Test health", open the idle mock thread, send a prompt, and verify that a new turn appears.
...
12. To probe Desktop bridging, stop the backend, set `CODEX_CONNECTION_MODE=proxy`, restart `pnpm dev:backend`, then tap `Test health` in Settings. If the bridge is `connected`, create a harmless Desktop-side approval and check the Inbox. If the bridge is `unavailable`, return to `CODEX_CONNECTION_MODE=child`; the current Codex Desktop install is not exposing a proxyable app-server control socket.
...
- Keep `BACKEND_AUTH_TOKEN` long and private.
