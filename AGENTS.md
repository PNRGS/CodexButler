...
- mobile notifications: approval notifications are on by default for registered devices; thread-level bell preferences sync to the backend and add idle notifications for opted-in threads. Notifications use generic butler text only (`Monsieur ? Your attention please.`, `Madame ? Your attention please.`, or `Your attention please.`) with no command, cwd, thread name, approval id, prompt, summary, or event type in local or push payloads. Push delivery uses Expo push as an opaque relay; validate in a dev build or alpha APK because Expo Go has notification limitations.
